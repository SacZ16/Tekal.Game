...
const textoCastellanoPrimera = ['Sabia que volverias, eres valiente! Bienvenido al modo de juego difícil. ','Esta vez, no verás repeticiones dentro del mismo nivel, sino que ciertos videos que aparecerán te fueron mostrados durante una partida anterior.','Cuando reconozcas uno de esos videos, ¡presiona ESPACIO!','Preparate y haz memoria. Trata de recordar lo que viste durante las partidas anteriores.','¿Estas listo? ¡A jugar!']

const textEnglishFirstLine = [`I knew you'd be back. You're one of the brave ones! Welcome to the hard game mode. `,`This time, you won't see repetitions inside a level; instead, some of the videos you'll be shown are videos that you've seen in a previous game.`,`Your objective is to detect those videos. When you see one, press SPACEBAR!`,`Concentrate and try to remember what you saw last time. You can do it!`,`Are you ready? Let's play!`]
...
counterpart.setLocale(lang); /* counterpart.setLocale(lang+''); */
  ...
  return (
    <div className='slideContainer'>
      ...
      {localStorage.getItem('lastScore')? <Link onClick={playWithOutLogin}><button onClick={localStorage.setItem('mode', 'video-lt')} style={{ backgroundColor:'transparent' ,fontSize: '25px', textTransform: 'uppercase', cursor: 'pointer', color: 'white', fontWeight: 'bold', borderStyle: "none", position: 'absolute',bottom:'5%', right:'5%' }}>{mode ? <Translate content="saltar" component="span" />:null}</button></Link>:null}
        ...
        {localStorage.getItem('idioma') === 'es' ? 
          ...
          <div className='tutorial_buttons'>
            ...
            {index < 4 ? <button className='right_btn_tutorial' style={index < 4 ? { color: 'white', cursor: 'pointer' } : { color: 'gray', cursor: 'default' }} onClick={index < 4 ? () => { setIndex(index + 1) } : null}>&#x276f;</button>
              : localStorage.getItem('lastScore')? <Link onClick={playWithOutLogin}><button onClick={localStorage.setItem('mode', 'video-lt')} style={{ fontSize: '25px', textTransform: 'uppercase', cursor: 'pointer', color: 'white', fontWeight: 'bold', borderStyle: "solid", borderColor: 'transparent', borderRadius: '5px', borderWidth: '2px', position: 'absolute' }}>{!mode ?  null : 'Start'}</button></Link>:null}
          </div>
          <div
